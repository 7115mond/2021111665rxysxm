...
def find_bridge(words,word_1,word_2):
    ...
    if num==1:
        print('The bridge words from '+ word_1+' to '+word_2+' is '+bridge[0])
    if num>1:
        i=0
        print('The bridge words from '+word_1+' to '+word_2+' are ',end='')
        while i<num:
            if i==num-1:
                print(bridge[i],end='.')
            else:
...
def get_bridge(words,word_1,word_2):
    i=0
    j=0
    num=0
    test = []
...
#find_bridge(words,'to','strange')
#print(txt_make('text_insert.txt',words))
min_way(words,'new','strange')
rande_walk(words)
